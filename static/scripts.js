document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ask user if they want to autoload the last backup
    const autoload = confirm('Do you want to autoload the last backup?');
    if (autoload) {
        fetch('/autoload_backup')
            .then(response => response.json())
            .then(data => {
                if (data.error) {
                    alert(data.error);
            ...
            .catch(error => console.error('Error autoloading backup:', error));
    }
