...
def load_attendees_from_csv(file):
    ...
    for row in reader:
        attendees.append({
            ...
            'check_in_time': row['check_in_time'] if row['check_in_time'] else None
        })
...
def backup_attendees():
    backup_file = os.path.join(backup_folder, f'archive_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv')
...
def autoload_backup():
    ...
    if backups:
        ...
        return jsonify({'message': 'Backup list loaded successfully', 'attendees': attendees, 'loaded_file': latest_backup})
    else:
        return jsonify({'message': 'No backups available, please look into /dist/backups folder inside the project', 'attendees': []})

@app.route('/save_attendees', methods=['POST'])
def save_attendees():
    try:
        backup_file = os.path.join(backup_folder, f'archive_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv')
        with open(backup_file, 'w') as f:
        ...
        print(f"Backup saved to {backup_file}")  # Debug statement
        manage_backups()
        return jsonify({'message': 'The list has been saved successfully in /dist/backups'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
